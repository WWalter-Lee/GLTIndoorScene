...
class ThreedFutureDataset(object):
    ...
    def _filter_objects_by_label(self, label):
        return [oi for oi in self.objects if oi.label == label]  # elf.objects:  # 从pkl家具中读取而来

    def get_closest_furniture_to_box(self, query_label, query_size):
        objects = self._filter_objects_by_label(query_label)
        if len(objects) == 0:  # 如果遇到家具中没有的类别（都是有新的前缀），就用类别的最后一个单词表示
            objects = self._filter_objects_by_label(query_label.split("_")[-1])
        mses = {}
