...
class BaseDataset(Dataset):
    ...
    def floor_plan_with_limits(limit_x, limit_y, axis=[0, 2]):
        def inner(scene):
            ...
            t_x = max_bbox[axis[0]] - min_bbox[axis[0]]  # max_bbox[0]-min_bbox[0]，floor_plan_bbox的x最大差值
            t_y = max_bbox[axis[1]] - min_bbox[axis[1]]  # max_bbox[2]-min_bbox[2]，floor_plan_bbox的z最大差值
    ...
    def with_generic_classes(box_types_map):
        '''
        将3D-FRONT场景中的家具类别映射为3D-FUTURE中的模型类别
        '''
        def inner(scene):
    ...
    def walls_check():
        def inner(scene):
            if len(scene.walls) == 0:  # 没有墙的信息
                return False
            if len(scene.walls["length"]) <= 2:  # 墙的面数不能小于2
                return False
            return scene
        return inner

    @staticmethod
    def with_object_types(objects):  # objects指3D-FRONT中的家具类别
        '''
        检查场景的家具类别是否在3D-FRONT中有定义
        '''
        def inner(scene):
            return (
                scene if all(b.label in objects for b in scene.bboxes)  # scene.bboxes.label也是3D-FRONT中的家具类别
                else False
            )
        return inner
...
class BaseScene(object):
    ...
    def __init__(self, scene_id, scene_type, bboxes):
        ...
        self.walls = None  # 加入墙的信息，在预处理数据存pkl的时候存入
