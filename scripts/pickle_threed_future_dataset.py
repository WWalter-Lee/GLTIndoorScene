...
def main(argv):
    ...
    parser.add_argument(
        "--without_lamps",
        action="store_true",  # 默认为False，如果不想要lamps，就要在参数中指定--without_lamps，由于是store_true,因此会指定为True
    ...
    # scenes_dataset应用了过滤器；pickle threed_front.pkl存的是所有类型的rooms(没有应用过滤器)
    # 下方pickle threed_future_model.pkl的对象是scenes_dataset中的物体(应用了过滤器)
